...
def test_cadex_v2_tx():
    test = bfh(V2_TX)
    deser = lib_tx_cadex.DeserializerCadex(test)
    tx = deser.read_tx()
    assert tx.version == 2
    assert tx.tx_type == 0
...
def test_cadex_tx_pro_up_serv_tx():
...
def test_cadex_tx_pro_up_rev_tx():
...
def test_cadex_tx_sub_tx_register_tx():
...
def test_cadex_tx_sub_tx_topup_tx():
...
def test_cadex_tx_sub_tx_reset_key_tx():
...
def test_cadex_tx_sub_tx_close_account_tx():
...
def test_cadex_tx_unknown_spec_tx():
...
def test_cadex_tx_wrong_spec_tx():
...
def test_cadex_tx_serialize_wrong_tx_type():
    test = bfh(CB_TX)
    deser = lib_tx_cadex.DeserializerCadex(test)
    tx = deser.read_tx()
    assert tx.tx_type == 5
    ...
    with pytest.raises(ValueError) as excinfo:
        ser = tx.serialize()
    assert ('Cadex tx_type does not conform'
