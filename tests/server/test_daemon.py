...
from electrumx.lib.coins import BitcoinCash, CoinError, Bitzeny, Cadex
...
@pytest.fixture(params=[Cadex])
def cadex_daemon(request):
    coin = request.param
    return coin.DAEMON(coin, ','.join(urls))
...
@pytest.mark.asyncio
async def test_protx(cadex_daemon):
    protx_hash = 'deadbeaf'
    with ClientSessionGood(('protx', ['info', protx_hash], {})):
        assert await cadex_daemon.protx(['info', protx_hash]) == {}
